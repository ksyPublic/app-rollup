...
const getOptions = (target, UI, options) => {
  ...
  if (
    target.getAttribute("data-toggle") === "group" &&
    UI.NAME === "ui.accordion"
  ) {
    _options = { ..._options };
  }
  return _options;
    ...
    on: {
      init: function () {
        ...
        [].forEach.call(slides, (el, i) => {
          const elTarget = options.a11yTarget
            ? el.querySelector(options.a11yTarget)
            : el;
...
if (window.HitaiUI) {
  ...
  console.warn("");
  console.warn(
    "   이 경고 문구는 [ui.common.js] 파일이 중복으로 로드되는 경우에 출력됩니다."
  );
